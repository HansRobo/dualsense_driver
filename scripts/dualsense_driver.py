...
class DualSenseDriver:
    def __init__(self):
        ...
        self.dualsense.init()
        self.joy_pub = rospy.Publisher('/dualsense/joy', Joy, queue_size=10)

    def update(self):
        joy = Joy()
        joy.axes.clear()
        joy.axes.append(self.dualsense.state.LX)
        joy.axes.append(self.dualsense.state.LY)
        joy.axes.append(self.dualsense.state.RX)
        joy.axes.append(self.dualsense.state.L2)
        joy.axes.append(self.dualsense.state.R2)
        joy.axes.append(self.dualsense.state.LY)
        joy.axes.append(0.0)
        joy.axes.append(0.0)
        joy.axes.append(0.0)
